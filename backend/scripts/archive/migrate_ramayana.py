"""
Migrate New Ramayana Chapters to Database

This script:
1. Backs up existing Ramayana data (optional restore)
2. Parses the 7 new markdown chapter files
3. Replaces existing Ramayana story with new chapters and scenes
...
CHAPTER_FILES = {
    1: "storytelling_analysis.md",  # Contains Chapter 1 (refined)
    2: "ramayana_chapter_02.md",
    3: "ramayana_chapter_03.md",
    4: "ramayana_chapter_04.md",
    5: "ramayana_chapter_05.md",
    6: "ramayana_chapter_06.md",
    7: "ramayana_chapter_07.md"
}


def extract_chapter_1_from_analysis():
    """Extract Chapter 1 from the storytelling_analysis.md file"""
    filepath = ARTIFACTS_DIR / "storytelling_analysis.md"
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Extract the sample chapter section
    match = re.search(r'# 📖 Chapter 1: The King Without an Heir\n\n(.*?)## 🎭 End of Chapter 1', 
                     content, re.DOTALL)
    if not match:
        raise ValueError("Could not find Chapter 1 in storytelling_analysis.md")
    
    chapter_content = match.group(0)
    return parse_chapter_content(chapter_content, chapter_num=1)
...
def parse_chapter_content(content, chapter_num):
    ...
    short_summary = f"{mood} • {reading_time}" if mood and reading_time else mood or reading_time or "A chapter from the Ramayana"
...
def detect_emotion_from_mood(mood_text):
    ...
    if 'peace' in mood_lower or 'calm' in mood_lower or 'serene' in mood_lower:
        ...
        return 'karuna'
    elif 'anger' in mood_lower or 'rage' in mood_lower or 'fury' in mood_lower:
        return 'raudra'
    elif 'fear' in mood_lower or 'terror' in mood_lower or 'dread' in mood_lower:
        return 'bhayanaka'
    elif 'love' in mood_lower or 'devotion' in mood_lower or 'romance' in mood_lower:
        return 'shringara'
    elif 'wonder' in mood_lower or 'awe' in mood_lower or 'marvel' in mood_lower:
        return 'adbhuta'
...
def backup_existing_ramayana(session):
    """Print existing Ramayana data for backup purposes"""
    story = session.exec(select(Story).where(Story.slug == "ramayana")).first()
    
    if story:
        print(f"\n📦 BACKUP: Found existing Ramayana story (ID: {story.id})")
        ...
        print("\n📦 No existing Ramayana found. Will create new.\n")
        return False


def delete_existing_ramayana(session):
    """Delete existing Ramayana story and all related data"""
    story = session.exec(select(Story).where(Story.slug == "ramayana")).first()
    
    if story:
        print(f"🗑️  Deleting existing Ramayana (ID: {story.id})...")
...
def migrate_new_ramayana():
    """Main migration function"""
    print("\n" + "="*60)
    print("🕉️  RAMAYANA MIGRATION - New Cinematic Storytelling")
    print("="*60 + "\n")
    ...
    with Session(engine) as session:
        # Step 1: Backup check
        backup_existing_ramayana(session)
        
        # Step 2: Delete old data
        delete_existing_ramayana(session)
        
        # Step 3: Create new story
        print("📚 Creating new Ramayana story...")
        story = Story(
            title="The Ramayana",
            slug="ramayana",
            category="Epic",
            description="An ancient tale of light and shadow, love and sacrifice, told as if you are living inside it. Not a myth. A mirror.",
            cover_image_url="https://pollinations.ai/p/cinematic%20painting%20of%20Ram%20and%20Sita%20in%20ancient%20India%20golden%20hour%20rich%20heritage?width=1080&height=1920&nologo=true",
            total_chapters=7
        )
        session.add(story)
        session.commit()
        ...
        for chapter_num in range(1, 8):
            ...
            # Parse chapter
            if chapter_num == 1:
                chapter_data = extract_chapter_1_from_analysis()
            else:
        ...
        total_scenes = session.exec(
            select(Scene).join(Chapter).where(Chapter.story_id == story.id)
        ).all()
        
        story.total_scenes = len(total_scenes)
        session.add(story)
        session.commit()
        ...
        print(f"\n🎯 Next Steps:")
        print(f"   1. Run verification: python verify_ramayana_migration.py")
        print(f"   2. Test API: curl http://localhost:8000/api/stories/")
        print(f"   3. View in frontend: npm run dev")
        print()


if __name__ == "__main__":
    try:
        migrate_new_ramayana()
